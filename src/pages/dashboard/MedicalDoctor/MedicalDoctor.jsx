...
const MedicalDoctorDashboard = () => {
  ...
  const loadDashboard = useCallback(async (doctorId) => {
    ...
    try {
      ...
      const mapped = [...data]
        ...
        .map((item) => {
          ...
          const displayStatus =
            ...
            item.assignedDepartment !== "medical" &&
      ...
      const appointmentPatientIds = new Set(
        appointmentPatients.map((item) => item.id),
      );
  ...
  const handleSendToPharmacy = async (appointmentId) => {
    const medication = window.prompt(
      "Enter medication name (e.g. Amoxicillin 500mg):",
    );
    if (!medication) return;
    const pharmacyNotes = window.prompt(
      "Enter pharmacy instructions or notes:",
    );
            ...
            <div>
              ...
              {user?.id && (
                <p className="text-xs text-gray-500 mt-1">
                  Doctor ID: {user.id} • Role: {user.role}
                </p>
              )}
            </div>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {recentAppointments.length === 0 ? (
                  <tr>
                    ...
                    >
                      No patient appointments yet. Ensure you are logged in as
                      the correct doctor account.
                      {user?.id && (
                        <div className="mt-2 text-xs text-gray-400">
                          Current doctor ID: {user.id}
                        </div>
                      )}
                    </td>
                  </tr>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-right">
                          ...
                          appointment.status === "scheduled" && (
                            <>
                              <button
                                ...
                                onClick={() =>
                                  handleSendToLaboratory(appointment.id)
                                }
                              >
                                Send to Lab
                              </button>
                              <button
                                ...
                                onClick={() =>
                                  handleReferToSpecialist(appointment.id)
                                }
                              >
                                Refer Specialist
                              </button>
                        ...
                        {appointment.assignedDepartment === "laboratory" && (
                          <button
                            ...
                            onClick={() =>
                              handleSubmitLabResults(appointment.id)
                            }
                          >
                            Return from Lab
                          </button>
                        ...
                        {appointment.assignedDepartment === "medical" &&
                          appointment.labResults && (
                            <button
                              ...
                              onClick={() =>
                                handleSendToPharmacy(appointment.id)
                              }
                            >
                              Send to Pharmacy
                            </button>
