...
class AppointmentController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        if ($request->has("doctor_id")) {
            $query->where("doctor_id", $request->doctor_id)->visibleToDoctor();
        }

        // Filter by specialist referral or specialist doctor if provided
        if (
            $request->has("specialist_id") ||
            $request->has("referred_specialist_id")
        ) {
            $specialistId = $request->input(
                "specialist_id",
                $request->input("referred_specialist_id"),
            );
            $query->where(function ($subQuery) use ($specialistId) {
                $subQuery
                    ->where("referred_specialist_id", $specialistId)
    ...
    public function store(Request $request): JsonResponse
    {
        try {
            $validated = $request->validate([
                ...
                "assigned_department" =>
                    "nullable|in:medical,laboratory,pharmacy,specialist,imaging",
                "status" =>
                    "nullable|in:scheduled,waiting,completed,cancelled,postponed",
            ...
            $conflictQuery = Appointment::where(
                "doctor_id",
                $validated["doctor_id"],
            )
            ...
            if ($conflictQuery->exists()) {
                return response()->json(
                    [
                        "message" =>
                            "You already have an appointment with this doctor at the selected date/time.",
                        "errors" => [
                            "appointment" => [
                                "Duplicate appointment for same doctor at this time.",
                            ],
                        ],
                    ],
                    422,
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    public function cancelDoctorAppointmentsDueToEmergency(
        int $doctorId,
        Request $request,
    ): JsonResponse {
        try {
            ...
            if (!in_array($doctor->role, ["medical_doctor", "specialist"])) {
                return response()->json(
                    [
                        "message" =>
                            "Only doctors can trigger emergency appointment cancellations.",
                    ],
                    403,
                );
            }
            ...
            $reason = $request->input(
                "reason",
                "Doctor emergency: appointments cancelled until emergency is resolved.",
            );
            ...
            if ($appointments->isEmpty()) {
                return response()->json(
                    [
                        "message" =>
                            "No scheduled appointments found to cancel.",
                    ],
                    200,
                );
            }
            ...
            return response()->json(
                [
                    "message" =>
                        "Failed to cancel appointments: " . $e->getMessage(),
                ],
                500,
            );
        }
    ...
    {
        $appointment = Appointment::with(["doctor", "patient"])->findOrFail(
            $id,
        );
    ...
    public function update(int $id, Request $request): JsonResponse
    {
        try {
            ...
            $validated = $request->validate([
                "status" =>
                    "sometimes|required|in:scheduled,waiting,completed,cancelled,postponed,expired",
                "appointment_date" => "sometimes|date|after_or_equal:today",
                "assigned_department" =>
                    "sometimes|required|in:medical,laboratory,pharmacy,specialist,imaging",
                ...
                if ($specialist && $specialist->role !== "specialist") {
                    return response()->json(
                        [
                            ...
                            "errors" => [
                                "referred_specialist_id" => [
                                    "Selected user must be a specialist.",
                                ],
                            ],
                        ],
                        422,
            ...
            if (array_key_exists("pharmacy_medication", $validated)) {
                $appointment->pharmacy_medication =
                    $validated["pharmacy_medication"];
            }

            if (array_key_exists("pharmacy_dosage", $validated)) {
                $appointment->pharmacy_dosage =
                    (string) $validated["pharmacy_dosage"];
            }
            ...
            if (array_key_exists("referred_specialist_id", $validated)) {
                $appointment->referred_specialist_id =
                    $validated["referred_specialist_id"];
            }
            ...
            $labRecord = Laboratory::firstOrNew([
            ...
            $labRecord->test_type =
                $appointment->reason ??
                ($appointment->symptoms ?? "Laboratory task");
            ...
            if (
                $previousAssignedDept === "laboratory" &&
                $appointment->assigned_department === "medical"
            ) {
                $labRecord->status = "completed";
            }
            ...
            if (
                $appointment->assigned_department === "imaging" ||
                $previousAssignedDept === "imaging"
            ) {
                $imagingRecord = Imaging::firstOrNew([
                    "appointment_id" => $appointment->id,
                ]);
    ...
    public function postpone(int $id, Request $request): JsonResponse
    {
        try {
            ...
            return response()->json(
                [
                    "message" =>
                        "Failed to postpone appointment: " . $e->getMessage(),
                ],
                500,
            );
        }
